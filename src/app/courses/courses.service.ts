...
export class CoursesService {
  ...
  getCourse(){
    return this.api.getUrl(`${this.baseUrl}${this.url}`).map(res=>res.json().courses).do(res=>console.log(res)).catch(this.handleServerError);
  ...
  // }
  getCoursedetail(id:number){
   return this.getCourse().map(courses=>courses.find(course => course.id == id));
  }
